...
class ARCHModel():
    ...
    def _loglikelihood(
        ...
        sigma1, sigma2 = self.volatility.compute_variance(
            np.append(vp,nu), resids, sigma1, sigma2, backcast, var_bounds
        )
        # 3. Compute log likelihood using Distribution
        llf = self.distribution.loglikelihood(np.append(mp,nu), resids, sigma1, sigma2, weight, individual)
        if not individual:
    ...
    def fit(
        ...
        sigma1, sigma2 = v.compute_variance(sv_volatility, resids, sigma1, sigma2, backcast, var_bounds)
        ...
        con = lambda x: (x[2] + x[4])*x[6] + (x[3] + x[5])*(1 - x[6])
        ...
        with open("weightedSigma_data", "wb") as fp:   #Pickling
            pickle.dump(weight* sigma1 + (1 - weight) * sigma2, fp)
        ...
        names = ["mean", "omega", "alpha1", "alpha2","beta1", "beta2", "weight", "nu"]
