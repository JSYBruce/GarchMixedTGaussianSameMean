...
class ARCHModelResult(ARCHModelFixedResult):
    ...
    def summary(self) -> Summary:
        ...
        def generateVaR(precision, distribution, params):
            for i in (np.arange(-10,10,precision)):
                if (distribution.cdf(params[-2], i, params[-1])-0.05) > 0:
                    if (0.05+ precision - distribution.cdf(params[-2], i, params[-1]))>0 :
                        return i
